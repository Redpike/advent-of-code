...
def generate_grid(depth, corner, target):
    grid = {}
...
def solve(depth, target, part):
    grid = generate_grid(depth, target, target)

    if part == 1:
        return sum([v[2] for v in grid.values()])
    else:
        corner = (target[0] + 100, target[1] + 100)
        grid = {c: v[2] for c, v in (generate_grid(depth, corner, target)).items()}
        return dijkstra(grid, corner, target)


def main():
    depth, target = read_input()
    print('Day 22 Part 1:', solve(depth, target, part=1))
    print('Day 22 Part 2:', solve(depth, target, part=2))
